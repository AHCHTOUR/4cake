...
function toggleLang(){
  ...
  window.location.href = '/ar/academie/pro/index.html';
}
...
function openRecFiche2(groupe, i){
  ...
  document.getElementById('ficheCard').innerHTML =
    ...
    '</div><div><ol>' + stepsHtml + '</ol></div></div>' + encartSciencePro(r);
  document.getElementById('ficheOverlay').classList.add('open');
}
...
function nutritionHTML(r){
  ...
  return `
    ...
    <div class="nut-panel">
      ...
      <p class="nut-note"><a href="/fr/academie/pro/calculateur.html" style="color:#8A6A0A;">🍽 Calculer votre propre recette →</a></p>
    </div>`;
}
...
const CATALOGUE_URL_PRO = '/fr/index.html';
...
function encartSciencePro(r){
  ...
  const tags = liste.map(id => {
    ...
    if(!t) return '';
    return `<a href="/fr/academie/pro/science/${id}.html" onclick="dlPush('clic_science_depuis_fiche_pro',{fiche:${JSON.stringify(titre)},notion:'${id}'})">${L(t.fr, t.ar)}</a>`;
  }).join('');
  if(!tags) return '';
  return `
